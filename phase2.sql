...
-- 4) For each store, which drink is least ordered (which has the most stock)
    SELECT st_name, d_brand FROM store, drink WHERE d_storekey = st_storeID GROUP BY st_name HAVING MAX(d_stock);
...
-- 12) Find the topping with the least stock for each store
    SELECT st_name, t_name FROM topping, store WHERE t_storekey = st_storeID GROUP BY st_name HAVING MIN(t_stock);
