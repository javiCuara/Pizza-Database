...
-- 5) For each customer that Dominos delivers to, find their order (entree, drink, side)
    SELECT e_name, d_brand, s_name FROM entree, side, drink, Orders, store, Delivery WHERE order_id = st_orderkey AND st_orderkey = o_orderkey AND o_custkey = c_custkey AND o_drink = d_key AND o_entree = e_key AND o_side = s_key AND st_name = "Dominos" GROUP BY c_name;

-- 6) Take the inventory of each side, entree ingredient, and drink for each store that offers tea
    SELECT COUNT(d_stock), COUNT(s_stock), COUNT(t_stock), COUNT(cr_stock), COUNT(sc_stock) FROM drinks, sides, toppings, (SELECT st_storekey AS id FROM store, drink WHERE d_type = "tea" AND d_storekey = st_storekey) as q WHERE d_storekey = q.id AND s_storekey = q.id AND t_storekey = q.id GROUP BY q.id;
...
-- 12) Find the topping with the least stock for each store
    SELECT st_name, t_name FROM topping, store WHERE t_storekey = st_storekey GROUP BY st_name HAVING MIN(t_stock);
